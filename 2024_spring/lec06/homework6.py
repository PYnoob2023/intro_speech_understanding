...
def minimum_Fs(f):
    ...
    '''
    Fs = 2 * f  # Nyquist rate: at least twice the frequency
    return Fs
...
def omega(f, Fs):
    ...
    '''
    omega = 2 * np.pi * f / Fs  # omega = 2 * pi * f / Fs
    return omega
...
def pure_tone(omega, N):
    ...
    '''
    n = np.arange(N)  # create an array from 0 to N-1
    x = np.cos(omega * n)  # calculate cos(omega * n) for each n
    return x
